...
Proof.
  ...
  change (config_mops (opcode_config BrIf i)) with 
    (etable_values cond_is_not_zero_cell i * etable_values keep_cell i).
  ...
  - destruct (cond_is_not_zero_cell_bit i) as [Hc | Hc]; rewrite Hc.
...
Lemma config_opcode_inj_BrIf : forall i instr,
    config_opcode (opcode_config BrIf i) =
            opcode_of_instruction instr ->
    instr =
      IBrIf (bool_of_Z (etable_values keep_cell i))
...
Proof.
  ...
  rewrite (itable_lookup_encode i BrIf Hrange Henabled Hops) in Hencode.
  ...
  subst.
  apply (config_opcode_inj_BrIf _ _ Hopcode).
Qed.
...
Proof.
  ...
  pose(H := op_br_if_cond_bit i Hrange).
  simpl in H.
...
Lemma brif_mops : forall i,
    0 <= i ->
    etable_values enabled_cell i = 1 ->
    etable_values (ops_cell BrIf) i = 1 ->
    mops_at_correct i ->
    (etable_values keep_cell i = 1 /\ etable_values cond_is_not_zero_cell i = 1 ->
    ...
    (etable_values keep_cell i = 0 \/ etable_values cond_is_not_zero_cell i = 0 ->
...
Proof.
  ...
  replace (class_of_row i) with BrIf in Hops.
  2: {
    symmetry.
    rewrite (proj1 (class_of_row_op i BrIf Hrow_enabled)); auto.
  }
  simpl in Hops.
  pose (mops_at_nonnegative (etable_values eid_cell i) MTableModel.LocationType_Stack).
  pose (mops_at_nonnegative (etable_values eid_cell i) MTableModel.LocationType_Heap).
...
Proof.
  intros i st xs Hrange Henabled Hmops Hops Hrel Hstk.
  assert (Heid := eid_nonzero i Hrange Henabled).
  apply (brif_mops) in Hmops; auto.
  ...
  assert (Hc: etable_values cond_cell i = 0).
  {
    eapply stack_rel_read_1_without_value with 
      (is_i32 := fun get => 1)
      (enable := fun get => get (ops_cell BrIf))
      (value := fun get => get cond_cell); auto.
    - eauto.
    - eauto.
  ...
  constructor.
  - rewrite iid_change with (idx := BrIf); auto.
    rewrite fid_change with (idx := BrIf); auto.
    simpl.
    rewrite Hc, H0.
  ...
  - rewrite eid_change by auto.
    rewrite sp_change with (idx := BrIf); auto.
    simpl.
  ...
  - rewrite eid_change by auto.
    ...
    rewrite (mpages_change i BrIf) by auto. simpl. rewrite Z.add_0_r.
  ...
  - rewrite callstack_update_stack, callstack_incr_iid.
    rewrite (frame_id_change i BrIf); simpl; auto.
    rewrite (fid_change i BrIf); simpl; auto.
    destruct Hrel. auto.
Qed.
...
Proof.
  intros i st xcond xd xs Hrange Henabled Hmops Hops Hrel Hstk Hcond Hdrop Hkeep.
  assert (Heid := eid_nonzero i Hrange Henabled).
  apply (brif_mops) in Hmops; auto.
  destruct Hmops as [_ Hmops].
  ...
  - rewrite eid_change by auto.
    rewrite sp_change with (idx := BrIf); auto.
    change (config_sp_diff (opcode_config BrIf i)) with
      (1 + etable_values cond_is_not_zero_cell i * etable_values drop_cell i).
    rewrite Hcond, Z.mul_1_l.
    apply state_stack_rel in Hrel.
    rewrite Hstk in Hrel.
    simpl in Hrel.
    destruct Hrel as [_ Hrel'].
...
Proof.
  intros i st xcond xv xd xs Hrange Henabled Hmops Hops Hrel Hstk Hcond Hdrop Hkeep.
  assert (Heid := eid_nonzero i Hrange Henabled).
  apply (brif_mops) in Hmops; auto.
  ...
  {
    eapply stack_rel_read_2_without_value with 
      (is_i32 := fun get => get is_i32_cell)
      (enable := fun get => get (ops_cell BrIf) * get keep_cell
                          * get cond_is_not_zero_cell)
  ...
  - rewrite eid_change by auto.
    ...
    eapply stack_rel_write_without_value_large_drop with
      ...
      (enable:= (fun get => get (ops_cell BrIf) * (get keep_cell) * (get cond_is_not_zero_cell)))
      (is_i32:= (fun get => get is_i32_cell))
